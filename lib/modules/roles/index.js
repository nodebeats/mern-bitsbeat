...
exports.addRole = async (req, res, next) => {
  try {
    ...
    if (!validation.length > 0) {
      ...
      if(findRole){
        res.send(config.messageConfig.role.roleConflictMsg);
      }else{
...
exports.readRole = async (req, res) => {
  try {
    ...
    if (roleList.length > 0) {
      ...
      res.send(config.messageConfig.role.roleReadMsg);
    }
  } catch (err) {
    console.log(err);
  }
...
exports.UpdateById = async (req, res, next) => {
  try {
    ...
    const putById = await db
      ...
      res.send(config.messageConfig.role.roleUpdateMsg);
  } catch (err) {
    console.log(err);
  }
};
...
exports.deleteById = async (req, res, next) => {
  try {
    ...
    res.send(config.messageConfig.role.roleDeleteMsg);
  } catch (err) {
    console.log(err);
  }
};
