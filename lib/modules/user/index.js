...
var sendingProperties = async (receivedHost, receivedEmail) => {
    try {
        ...
        redisHelper.expireKey(emailToken, 300);
        ...
        if (!validation.length > 0) {
            ...
            if (checkEmail) {
                ...
                if (agree_terms_condition === true) {
                    try {
                        await saveUser({ first_name, last_name, email, password, salutation, user_role, agree_terms_condition, deleted, added_on, isVerified });
...
exports.resendToken = async (req, res) => {
    try {
        ...
        res.status(400).json({ status: "Bad Request", msg: err, msg_from:"resendToken" });
    }

}

exports.redisFetch = async (req,res) => {
    let id = await req.params.tokenId;
    console.log(id);
    let k = await redisHelper.fetchData(id);
    if(!k) {console.log("error");res.json({"msg":"error"})}
    console.log(k);
    res.json(k);
}