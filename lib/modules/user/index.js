const BCRYPT_SALT_ROUNDS = 10,
    ...
    multerHelper = require('../../helpers/multer.helper');
...
var checkValidation = async (req) => {
    ...
    req.checkBody('first_name', configMessage.messageConfig.user.validationErrMessage.first_name_alpha).isAlpha()//check if first_name is only string and not number
    ...
    req.checkBody('salutation', configMessage.messageConfig.user.validationErrMessage.salutation).notEmpty();
    req.checkBody('salutation', configMessage.messageConfig.user.validationErrMessage.salutationField).optional().isIn(['Mr.', 'Mrs.', 'Ms.']);
            ...
            if (checkEmail) {
                ...
                if (agree_terms_condition === true) {
                    try {
                        ...
                        let k = await saveUser({ first_name, last_name, email, password, salutation, user_role, agree_terms_condition, deleted, added_on, isVerified, subscribe, image_path });
