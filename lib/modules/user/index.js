...
var checkValidation = async (req) => {
    ...
    req.checkBody('user_role', configMessage.messageConfig.validationErrMessage.user_role_field).optional().isIn(['superuser', 'enduser', 'general']);
...
const sendingProperties = async (receivedHost, receivedEmail, receivedReq, receivedRes) => {
    try {
        if (receivedReq.route.path === "/subscribe/anonymous") {
            ...
            const link = "http://" + receivedHost + "/verify?id=" + emailToken;
