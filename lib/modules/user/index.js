        ...
        if (!validation.length > 0) {
            ...
            if (checkEmail) {
                ...
                if (agree_terms_condition === true) {
                    collection.insertOne({ first_name, last_name, email, password, salutation, user_role, agree_terms_condition, deleted, added_on })
                        .then((data,err) => {
                            if(err) {return res.status(500).send({ msg: err.message });}

                            // let tokenBcrypt = bcrypttHelper.hashPwd(email,salt);
                            // tokenCollection.insertOne({_userId:_id,token:tokenBcrypt})
