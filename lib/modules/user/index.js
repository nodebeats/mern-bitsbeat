...
var checkValidation = async (req) => {
    ...
    req.checkBody('user_role', configMessage.messageConfig.validationErrMessage.user_role_field).optional().isIn(['sueruser', 'enduser']);
        ...
        if (!validation.length > 0) {
            ...
            if (checkEmail) {
                ...
                if (agree_terms_condition === true) {
                    ...
                    //     res.json(err);
                    // });
                    save.saveUser({ first_name, last_name, email, password, salutation, user_role, agree_terms_condition, deleted })
                        .then((data) => {
                            res.status(200).json({
                                status_code: "200",
                                status: "Ok",
                    ...
                    res.status(201).json({
                        status_code: "201",
                        status: "Created",
                        message: configMessage.messageConfig.user.agree_terms_condition
                    });
                }
