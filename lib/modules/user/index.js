...
var checkValidation = async (req) => {
    ...
    req.checkBody('password',configMessage.messageConfig.validationErrMessage.passwordCharacter).matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})");
            ...
            if (checkEmail) {
                ...
                if (agree_terms_condition === true) {
                    try {
                        ...
                        redisHelper.saveToRedis(emailToken,{user_id: stringA, created_at: new Date()},'EX',608400);

                        let sth = await redisHelper.fetchData(emailToken);
...
exports.verifyEmail = async (req, res) => {
    ...
    let tokenVerify = await redisHelper.fetchData(req.params.token);
    console.log(JSON.parse(tokenVerify));
