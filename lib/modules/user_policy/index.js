...
const redi = require('./../../helpers/redis.helper');
...
exports.addPolicy = async (req, res, next) => {
  try {
    ...
    if (!validation.length > 0) {
      ...
      if (findPolicy) {
        res.status(409).json({
          status: config.messageConfig.policy.policyConflict
        });
      } else {

        let policy = {
          policy_title: req.body.policy_title.toUpperCase(),
          policy_route: req.body.policy_route,
          policy_method: req.body.policy_method,
    ...
    res.status(409).json({
      status: config.messageConfig.policy.policyError,
...
exports.updateById = async (req, res, next) => {
  try {
    ...
    res.status(200).json(config.messageConfig.policy.policySuccesful);
  } catch (err) {
    res.status(409).json({
      status: config.messageConfig.policy.policyError,
