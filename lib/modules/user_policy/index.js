...
exports.addPolicy = async (req, res, next) => {
      try {
        ...
        if (findPolicy) {
          ...
          res.json({message: "Policies created Successfully!"})
        }
      }catch (err) {
        ...
        exports.getPolicy = async (req, res) => {
          try {
            const policyList = await db.collection("policy").find({deleted: false }, {
                projection: {
                  _id: 1,
                  policy_title: 1,
                  policy_route: 1,
            ...
            if (policyList.length > 0) {
              ...
              res.json({
                message: "There is no any Policy list"
              });
            }
          } catch (err) {
            return res.status(400).json({
        ...
        exports.deleteById = async (req, res, next) => {
          try {
            ...
            let id = req.params.id;
              const patchById = await collection.updateOne({_id: ObjectID(id) },
               {
                $set: {
                  deleted: true
                }
